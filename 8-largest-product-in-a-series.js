/* The four adjacent digits in the 1000-digit number that have the greatest product are 9 × 9 × 8 × 9 = 5832. */
const thousandDigitNumber = "";
/* Find the thirteen adjacent digits in the 1000-digit number that have the greatest product. What is the value of this product? */

// const thousandDigitNumberString = thousandDigitNumber.toString()
// console.log(thousandDigitNumberString) // Infinity .. ?!
// made the number a string manually in the code for now

const tDN = thousandDigitNumber
const thousandDigitArray = []
const tDA = thousandDigitArray

const stringToArray = string => {
    for (let i = 0; i < string.length; i++) {
        tDA.push(string[i])
    }
}
stringToArray(tDN)
// console.log(tDA[0]) // 7
// console.log(tDA[999]) // 0
console.log(`initial number length: ${tDA.length}`)

const adjacentDigits = 4 // defines how many adjacent digits we want to find the highest product for
/*
0123456789
10 digits
2 adjacentDigits
9 arrays
= 10-2+1
3 adjacentDigits
8 arrays
= 10-3+1
4 adjacentDigits
7 arrays
= 10-4+1
9 adjacentDigits
10-9+1=2
*/
const numberOfAdjacentDigitArrays = tDA.length - 4 + 1
console.log(`number of adjacent digits: ${adjacentDigits}`)
console.log(`number of adjacent digit arrays: ${numberOfAdjacentDigitArrays}`)

const buildAdjacentDigitArrays = howManyArrays => {
    let adjacentDigitArrays
    for (let arrays = 0; arrays < howManyArrays; arrays++) {
        
    }
    return adjacentDigitArrays
}
buildAdjacentDigitArrays(numberOfAdjacentDigitArrays)
